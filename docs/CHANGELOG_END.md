...
- **① 附魔灵弓（双模式 + 锻造5选1本体）**：`EndSpiritBowMight` 在背包-本弓窗口提供一排开关(EndModeWand 契约):
  - **选本体(5选1)**：从正向附魔全池随机抽 5 个让你挑 1 个，写成本弓 `本体附魔`；
  - **模式 A「稳固本体」**：命中触发本体附魔，并借 `Weapon.Enchantment.genericProcChanceMultiplier` 对本模式(仅本弓、仅当局)叠加 **+50% 奥术等价**触发倍率，本体常必触发且>100% 溢出自动转强；
  - **模式 B「随机附魔」**：放弃本体，改为每击必触发一个全池【随机】附魔（含稀有）。
  模式状态随本弓 bundle 存档。
  - **收尾校准**：本体一经选定，本弓详情窗口里的「选本体(5选1)」按钮即消失(不再可反复刷本体)。
- **② 雷鸣灵弓（攻速 +50%）**：`EndSpiritBowStorm` 自身攻击速度 +50%（`speedMultiplier` ×1.5），射速比普通灵能弓快一半。
