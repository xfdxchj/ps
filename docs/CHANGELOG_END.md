...
## [WIP] 用户最新追加的需求（尚未提交完整，将在下一批一次做掉）
1. 便利测试挑战开时：**开局“日志全解锁”＝把各类物品全部标记为** `Catalog.setSeen`(已见过/发现),方便你在图鉴看自制宝石/进化法杖。
2. 便利挑战开局给一个**“召选道具”**（使用不消耗）—— 用一个不消耗的物品，用了可**选择并获得一件想要的物品**。
3. **删除“储物木桶”**（SPD 的收纳木桶/储物 item，疑似 trinket/某个 `Crate`/bin；先精确定位类名再移除其可获取途径）。
4.（框架建议）各 Evolved 法杖的**专属机制**、锻造/换装入口接入、余下挑战区域/无尽。
