...
/**
 * END(移植自魔绫·挑战区): 磷火燃烧（会烧毁卷轴/生肉，并在地面留下磷火）。
 * 移植调整：去掉魔绫的 Char.DamageType 参数；fx() 的 HALOMETHANEBURNING 精灵状态本 fork 无，故略去纯视觉部分。
 */
public class HalomethaneBurning extends Buff implements Hero.Doom {
    ...
    public boolean act() {
        ...
        if (target.isAlive() && !target.isImmune(getClass())) {
            ...
            int damage = Random.NormalIntRange( 2,4 + Dungeon.depth/5 + hero.lvl/5 );
            //初始伤害2到4点 随着每深入5层+1伤害 随着玩家等级每升级5级+1伤害
