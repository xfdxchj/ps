...
/**
 * END(移植自魔绫·挑战区): 空洞遗迹 NPC·静默魔女。
 * 移植调整：魔绫原版交互会打开 `WndDialog(SlicePlot)`（依赖未搬的对话/剧情节系统），
 * 此处改为等价的 `WndQuest` 文本对话，保留 NPC 的出现与交互。
 * TODO(待搬)：搬入 `custom/utils/plot/hollow/*` 与 `WndDialog` 后可恢复原剧情演出。
 */
    ...
    public boolean interact(Char c) {
        ...
        if(first && Dungeon.depth == 27 && Statistics.Hollow_Holiday){
            Game.runOnRenderThread(() -> GameScene.show(new WndQuest(this,
                    Messages.get(SliceGirl.class, "hello"))));
        }
