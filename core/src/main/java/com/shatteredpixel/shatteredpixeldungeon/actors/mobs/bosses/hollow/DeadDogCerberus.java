...
//END(移植自魔绫·挑战区): 冥犬 Boss。
//移植调整：魔绫有自造基类 actors/Boss(含 initProperty/initBaseStatus/initStatus)，本 fork 无 → 改为 extends Mob 并直接赋值。
public class DeadDogCerberus extends Mob {
    ...
    {
        //END(移植调整): 原魔绫 initProperty()/initBaseStatus(20,60,20,26,1000,0,0)/initStatus(100) 的等价赋值
        defenseSkill = 26;   //原 eva
        HP = HT = 1000;      //原 ht
        EXP = 100;           //原 initStatus(100)
    ...
    public void notice() {
        ...
        if (!BossHealthBar.isAssigned()) {
            ...
            //END(移植调整): 本 fork 无 playBossMusic()/bossReady()
     ...
     * END(移植调整): 本 fork 的 Char.move(int) 是 final，无法 override → 逻辑移入 act()。 */
    private void wanderRegen() {
        if(state == WANDERING){
            HP += Math.min(3, HT - HP);
        }
    }
    ...
    public void onAttackComplete() {
        ...
        if(ComboAttackThis){
            //END(移植调整): GameScene.scene 非 public → 用 sprite.parent（本 fork 惯用法）
            sprite.parent.add(new Delayer(2f){
                @Override
    ...
    protected boolean act() {
        //END(移植调整): 原 move() override 的回血逻辑（本 fork 的 move 是 final）
        wanderRegen();
    ...
    public void die( Object cause ) {
        ...
        //END(移植调整): 省略死后生成的剧情 NPC Typhon（依赖未搬的剧情/对话系统）
    }
}