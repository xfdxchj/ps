...
public class TowerMindBad extends Mob {
    ...
    {
        HT = HP = 1;
        spriteClass = TowerBadMindSprite.class;
    ...
    protected boolean act() {
        ...
        if(repiaer && buff(MyCoreHeart.RepaierDown.class)==null && slowCoolDown <= 0){
            ((TowerBadMindSprite) sprite).ReActivate();
            for (Mob mob : Dungeon.level.mobs.toArray(new Mob[0])) {
                if(mob.distance(this) <= 50 && mob != this ){
                    if(Statistics.isOldDay){
                        Buff.prolong(mob, Vertigo.class, 8f);
                    }
                }
            }
            slowCoolDown = 30;
        ...
        if(buff(MyCoreHeart.RepaierDown.class)==null && repiaer && Dungeon.hero.buff(BlobImmunity.class) == null && blobCoolDown <= 0){
            Buff.prolong( hero, BlobImmunity.class, 10f );
            blobCoolDown = 40;
        }
        if(blobCoolDown >= 0){
            blobCoolDown--;
        ...
        if(buff(MyCoreHeart.RepaierDown.class)==null){
            if(Statistics.RepaierTowerCount>0 && !repiaer){
                Game.runOnRenderThread(new Callback() {
                    @Override
                    public void call() {
                        GameScene.show(new WndOptions(new TowerBadMindSprite(),
                                Messages.titleCase(Messages.get(TowerMindBad.class, "name")),
    ...
    public static class TowerBadMindSprite extends MobSprite {
        Animation activeIdle;
        public TowerBadMindSprite() {
            super();

            texture(Assets.Sprites.TowerMind);
        ...
        private int Rules() {
            int get = 0;
            for (Mob mob : Dungeon.level.mobs.toArray(new Mob[0])) {
                if (mob instanceof TowerMindBad) {
                    get = ((TowerMindBad) mob).repiaer ? 0 : 2;
                }
            }
            return get;
        }
