...
public class TrueInvisibiity extends FlavourBuff {
    ...
    public void fx(boolean on) {
        if (on) target.sprite.add( CharSprite.State.INVISIBLE /*END(移植调整):本fork无TRUE_INVISIBLE*/ );
        else if (target.invisible == 0) target.sprite.remove( CharSprite.State.INVISIBLE /*END(移植调整):本fork无TRUE_INVISIBLE*/ );
    }
