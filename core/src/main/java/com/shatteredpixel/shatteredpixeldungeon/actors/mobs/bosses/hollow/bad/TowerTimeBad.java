...
public class TowerTimeBad extends Mob {
        ...
        if(buff(MyCoreHeart.RepaierDown.class)==null){
            if(Statistics.RepaierTowerCount>0 && !repiaer){
                Game.runOnRenderThread(new Callback() {
                    @Override
                    public void call() {
                        GameScene.show(new WndOptions(new TowerTimeBadSprite(),
                                Messages.titleCase(Messages.get(TowerTimeBad.class, "name")),
    ...
    protected boolean act() {
        ...
        if(repiaer && buff(MyCoreHeart.RepaierDown.class)==null && slowCoolDown <= 0){
            ((TowerTimeBadSprite) sprite).ReActivate();
        ...
        if(buff(MyCoreHeart.RepaierDown.class)==null && repiaer && Dungeon.hero.buff(Haste.class) == null && blobCoolDown <= 0){
    ...
    public static class TowerTimeBadSprite extends MobSprite {

        Animation activeIdle;

        public TowerTimeBadSprite() {
            super();
        ...
        private int Rules() {
            int get = 0;
            for (Mob mob : Dungeon.level.mobs.toArray(new Mob[0])) {
                if (mob instanceof TowerTimeBad) {
                    get = ((TowerTimeBad) mob).repiaer ? 0 : 3;
                }
            }
            return get;
        }
