...
public class ShubNiggurath extends Boss {
    ...
    // 最大分裂次数
    private static final int MAX_SPLIT_COUNT = 9;
...

    {
        ...
        properties.add(Property.IMMOVABLE); //END(port): 本fork无 Property.UNKNOWN
    ...
    protected boolean act() {
        ...
        if(!notFirst){
            initProperty();
            Buff.affect(this, ChampionEnemy.Blazing.class); //END(port): 本fork无 Bomber
        } else {
            Buff.detach(this, ChampionEnemy.Blazing.class); //END(port): 本fork无 Bomber
        }
