...
public class TowerMachineBad extends Mob {
    ...
    protected boolean act() {
        ...
        if(repiaer && buff(MyCoreHeart.RepaierDown.class)==null && slowCoolDown <= 0){
            ((TowerMachineBadSprite) sprite).ReActivate();
            for (Mob mob : Dungeon.level.mobs.toArray(new Mob[0])) {
                if(mob.distance(this) <= 50 && mob != this ){
                    if(Statistics.isOldDay){
                        Buff.prolong(mob, Vulnerable.class, 7f);
                    }
                }
            }
            slowCoolDown = 30;
        ...
        if(buff(MyCoreHeart.RepaierDown.class)==null && repiaer && Dungeon.hero.buff(Adrenaline.class) == null && blobCoolDown <= 0){
        ...
        if(buff(MyCoreHeart.RepaierDown.class)==null){
            if(Statistics.RepaierTowerCount>0 && !repiaer){
                Game.runOnRenderThread(new Callback() {
                    @Override
                    public void call() {
                        GameScene.show(new WndOptions(new TowerMachineBadSprite(),
                                Messages.titleCase(Messages.get(TowerMachineBad.class, "name")),
    ...
    public static class TowerMachineBadSprite extends MobSprite {
        Animation activeIdle;
        public TowerMachineBadSprite() {
            super();
        ...
        private int Rules() {
            int get = 0;
            for (Mob mob : Dungeon.level.mobs.toArray(new Mob[0])) {
                if (mob instanceof TowerMachineBad) {
                    get = ((TowerMachineBad) mob).repiaer ? 0 : 2;
                }
            }
            return get;
        }
