...
public class DeathRongBoat extends MeleeWeapon {
    ...
    public int proc(Char attacker, Char defender, int damage) {
        if (Dungeon.level.water[attacker.pos] || Dungeon.level.map[attacker.pos] == Terrain.WATER /*END(移植调整):本fork无SALT_WATER*/) {
            int targetPos = defender.pos;
            for (Mob mob : Dungeon.level.mobs.toArray(new Mob[0])) {
                if (mob.pos == targetPos) continue;
                if (Dungeon.level.water[mob.pos] || Dungeon.level.map[mob.pos] == Terrain.WATER /*END(移植调整):本fork无SALT_WATER*/) {
                    if(mob.alignment == Char.Alignment.ENEMY && mob != attacker){
                        ...
                        mob.damage(finalDamage, attacker); //END(移植调整): 去掉 DamageType
                    }
                }
            }
        }
