...
public class ItemSpriteSheet {
	...
	private static final int TX_HEIGHT = 1024;   //END(图集扩展): 512->1024（2的幂！上次用640非2的幂导致图案错乱）
	...
	//END(修复+图集扩展): 原 fork 的 items.png 仅 256x512(32行)，放不下魔绫的传说武器图标。
	//已用工具 _tools/expand_items_atlas.py 把图集扩到 **256x640(40行)**：
	//  - y=0..31 原样保留（原版图标零改动，已校验像素差异=0）
	//  - y=32..38 透明填充
	...
	public static final int  SAPIUS         = LENGYWEAPONS+3;   //别名
	static {
	...
	private static final int DOCUMENTS  =                                   xy(1, 32);  //END: 图集已扩到64行, 用 y=32 空白行(不再与FOOD重叠)
