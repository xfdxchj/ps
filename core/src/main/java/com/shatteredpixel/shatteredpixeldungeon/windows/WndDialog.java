...
public class WndDialog extends Window {
    ...
    public WndDialog(Plot plot,Boolean regained) {
        ...
        if(true) //END(移植调整): 本fork landscape() 返回 boolean
        {
        ...
        chrome.x = MARGIN_X;
        chrome.y = (float) (PixelScene.uiCamera.height - 0f /*END:本fork无ToolbarHeight*/ - chromeHeight);
        add(chrome);
    ...
    public PointerArea makeArea() {
        PointerArea pointerArea = new PointerArea(0,0, PixelScene.uiCamera.width, PixelScene.uiCamera.height) //we wants to make you click anywhere to result in processing plot
        {
            ...
            //END(port note): @Override removed - base class has no such signature
    ...
    public void showChoice(Choice... existing) {
        ...
        if(landscape)
        {
            pos = 0f /*END:本fork无StatusHeight*/;
        }
        else
        {
            int half = existing.length / 2;
            float offset = (PixelScene.uiCamera.height - 0f /*END:本fork无ToolbarHeight*/ - 0f /*END:本fork无StatusHeight*/ ) / 2;
