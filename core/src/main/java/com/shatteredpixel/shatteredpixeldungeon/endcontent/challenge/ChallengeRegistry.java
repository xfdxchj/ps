...
public final class ChallengeRegistry {
	...
	static {
		...
		all.add(ChallengeDef.at(116, "精英强敌", "champion_enemies")
				.group("经典").tendency(ChallengeDef.TENDENCY_MONSTER).level(2)
				.tier(ChallengeDef.TIER_EASY).state(ChallengeDef.STATE_DONE)
				.legacyBit(legacy.CHAMPION_ENEMIES).build());
	...
	private static void registerPending(List<ChallengeDef> all) {
		...
		add(all, 10, "巨型化",     "giant",            "战斗", T_MON, 2, T_EASY,   "");
				...
				"玩家攻击提高 20%，生命上限降低 13%。");
		add(all, 13, "狂热",       "frenzy",           "战斗", T_MON, 2, T_EASY,   "");
		done(all, 14, "精英强化",   "elite_boost",      "战斗", T_MON, 2, T_EASY,   "s:4,75",
				"精英怪的生命上限、伤害、命中、闪避各提高 20%。");
		add(all, 15, "首领护卫",   "boss_guard",       "战斗", T_MON, 3, T_MED,    "");
		add(all, 16, "大力水手",   "popeye",           "战斗", T_TWO, 1, T_EASY,   "");
		add(all, 17, "情人节",     "valentine",        "战斗", T_BEN, 1, T_MED,    "");
		add(all, 18, "老龄化",     "aging",            "战斗", T_BEN, 3, T_MED,    "");
		add(all, 19, "风驰电掣",   "swift",            "战斗", T_TWO, 1, T_EASY,   "s:103");
		...
		add(all, 23, "血流成河",   "bloodbath",        "战斗", T_TWO, 1, T_EASY,   "");
		add(all, 24, "以牙还牙",   "retaliation",      "战斗", T_MON, 1, T_EASY,   "");
		...
		add(all, 30, "人口密集",   "crowded",          "怪物", T_MON, 2, T_EASY,   "s:75");
		add(all, 73, "神秘复苏",   "mystic_revival",   "怪物", T_MON, 2, T_MED,    "s:77,86");
		add(all, 75, "精英地牢",   "elite_dungeon",    "怪物", T_MON, 3, T_MED,    "s:14,30,97");
		...
		add(all, 119,"怪物浪潮",   "monster_wave",     "怪物", T_TWO, 2, T_EASY,   "s:30");
		add(all, 122,"我的世界II", "minecraft_ii",     "怪物", T_MON, 3, T_MED,    "s:97");

		//---- 经济 ----
		add(all, 33, "黑市",       "black_market",     "经济", T_TWO, 1, T_MED,    "");
		add(all, 34, "赏金制度",   "bounty",           "经济", T_BEN, 1, T_EASY,   "");
		add(all, 35, "丰饶",       "abundance",        "经济", T_BEN, 1, T_EASY,   "x:36");
		add(all, 36, "贫瘠",       "barren",           "经济", T_RES, 2, T_EASY,   "x:35");
		add(all, 37, "高价回收",   "high_buyback",     "经济", T_BEN, 1, T_EASY,   "x:32");
		...
		add(all, 43, "一贫如洗",   "destitute",        "经济", T_RES, 2, T_EASY,   "");
		add(all, 44, "慷慨商人",   "generous_merchant","经济", T_TWO, 1, T_EASY,   "s:32");
		...
		add(all, 47, "稀缺补给",   "scarce_supplies",  "药剂", T_RES, 2, T_EASY,   "x:48");
		add(all, 48, "过量补给",   "excess_supplies",  "药剂", T_BEN, 1, T_EASY,   "x:47");
		...
		add(all, 80, "冰天雪地",   "frozen_world",     "环境", T_TWO, 2, T_EASY,   "");
		add(all, 90, "雷暴",       "thunderstorm",     "环境", T_TWO, 2, T_EASY,   "");
		...
		//---- 特殊 / 娱乐 ----
		add(all, 61, "炸弹狂魔",   "bomb_fanatic",     "特殊", T_BEN, 1, T_EASY,   "");
		add(all, 62, "芙莉莲",     "frieren",          "特殊", T_BEN, 1, T_EASY,   "");
		add(all, 63, "鼠鼠可爱",   "cute_rats",        "特殊", T_NEU, 1, T_HARD,   "");
		add(all, 64, "宝物猎人",   "treasure_hunter",  "特殊", T_TWO, 1, T_EASY,   "");
		add(all, 65, "及时雨",     "timely_rain",      "特殊", T_BEN, 2, T_MED,    "s:104");
		add(all, 67, "宝箱危机",   "chest_crisis",     "特殊", T_RISK,3, T_MED,    "");
		add(all, 70, "生活部长",   "life_minister",    "特殊", T_NEU, 1, T_MED,    "");
		add(all, 71, "喝大了",     "drunk",            "特殊", T_TWO, 2, T_MED,    "");
		add(all, 72, "前程似锦",   "bright_future",    "特殊", T_NEU, 1, T_MED,    "");
		add(all, 95, "耗子尾汁",   "rat_tail_soup",    "特殊", T_BEN, 1, T_MED,    "");
		add(all, 96, "奥利给",     "oligei",           "特殊", T_TWO, 1, T_MED,    "");
		add(all, 104,"命悬一线",   "close_call",       "特殊", T_TWO, 2, T_MED,    "s:65,68");
		add(all, 118,"天意侵蚀",   "providence",       "特殊", T_TWO, 1, T_MED,    "");
		add(all, 120,"404",        "error_404",        "特殊", T_RISK,2, T_MED,    "");
		add(all, 123,"大学生",     "college_student",  "特殊", T_TWO, 1, T_EASY,   "");
		add(all, 134,"黄金蜂蜜酒", "golden_mead",      "特殊", T_TWO, 3, T_MED,    "s:128");
		add(all, 137,"奶龙大笑",   "milk_dragon",      "特殊", T_NEU, 1, T_MED,    "");
		...
		add(all, 130,"格林之音",   "grimm_music",      "格林", T_NEU, 1, T_SER,    "r:118,137;s:125,126,127,128,129,131,132");
